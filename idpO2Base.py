# oxygen_particle_sim.py
# --------------------------------------------------------------
# Interactive particle‑based oxygen simulation with Bayesian field
# reconstruction. Each simulated particle = one O₂ molecule. Users
# change parameters via text boxes and click **Start**. The colorbar
# adapts every GP refresh to span the current min–max inferred
# concentration.
# --------------------------------------------------------------
...
def sensor_concentrations(particles, sensors):
    centres = sensors[:, :2]
    radii = sensors[:, 2]
    inside = cdist(particles, centres) <= radii
    counts = inside.sum(axis=0)
    return counts / (np.pi * radii ** 2)  # molecules per m² (2‑D analogue)
...
def run_sim(fig, ax, L=10.0, n_particles=500, grid_res=50, n_steps=200,
            diff_coeff=0.1, dt=0.05):
    """Set up and launch the particle‑GP animation."""

    # Simulation / GP constants
    sigma = np.sqrt(2 * diff_coeff * dt)
    sensor_radius = 0.1 * L
    sensors = np.array([
    ...
    np.random.seed(42)
    particles = np.random.uniform(0, L, size=(n_particles, 2))

    # Grid of prediction points
    x_lin = np.linspace(0, L, grid_res)
    y_lin = np.linspace(0, L, grid_res)
    X_grid, Y_grid = np.meshgrid(x_lin, y_lin)  # 2 arrays, shape (grid_res, grid_res)
    ...
    ax.set_ylim(0, L)
    ax.set_aspect('equal')
    scat = ax.scatter([], [], s=5, c='white', edgecolors='k')
    # Start with zeros; scale updated dynamically
    ...
    ax.set_title('Oxygen particles & inferred concentration')

    # ---------------- Animation update ------------------------
    def update(frame):
        nonlocal particles
        # Brownian step + reflection
        particles += np.random.normal(scale=sigma, size=particles.shape)
        particles[:, 0] = reflect(particles[:, 0], L)
        particles[:, 1] = reflect(particles[:, 1], L)

        # Sensor readings
        conc = sensor_concentrations(particles, sensors)
